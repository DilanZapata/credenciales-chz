...
if ($id > 0) {
    permisoModel::exigir('users.update');
    $data      = respuestaVista(usuarioController::verController($id));
    $user      = $data['user'];
    $userRoles = array_column($data['roles'], 'id');
...
$companies   = catalogoModel::companies(false);
$locations   = catalogoModel::locations(null, false);
$departments = catalogoModel::departments(null, false);
?>
<?php
$isEdit = $user !== null;
